...
struct Highway {
	...
	Highway(const string& name, double miles, int speed_limit = 65, int lanes =
			4, bool divided = true) :
			name(name), miles(miles), speed_limit(speed_limit), lanes(lanes), divided(
					divided) {
	}
...
typename boost::graph_traits<Map>::vertex_descriptor do_add_vertex(Map& map,
		VertexIterator, const Bundle& bundle, truth<true>) {
	return add_vertex(bundle, map);
}

template<typename Map, typename VertexIterator, typename Bundle>
typename boost::graph_traits<Map>::vertex_descriptor do_add_vertex(Map& map,
		VertexIterator& vi, const Bundle& bundle, truth<false>) {
...
void test_io(adjacency_list<EL, VL, D, VP, EP, GP>& map, int) {
	...
	in >> read(map2);
	typename graph_traits<adjacency_list<EL, VL, D, VP, EP, GP> >::vertex_iterator v2 =
			vertices(map2).first;
...
void test_bundled_properties(Map*, truth<CanAddVertex> can_add_vertex) {
	...
	vertex_descriptor bloomington = do_add_vertex(map, vi,
			City("Bloomington", 39000, 47401), can_add_vertex);
	BOOST_CHECK(
	...
	edge_descriptor our_trip = add_edge(v, bloomington, Highway("Long", 1000),
			map).first;
...
void test_subgraph_bundled_properties() {
	typedef boost::subgraph<
			boost::adjacency_list<boost::vecS, boost::vecS,
					...
					boost::property<boost::edge_index_t, int, Highway> > > SubMap;
...
int test_main(int, char*[]) {
	typedef boost::adjacency_list<boost::listS, boost::vecS,
			boost::bidirectionalS, City, Highway> Map1;
