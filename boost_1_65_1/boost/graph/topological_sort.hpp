...
struct topo_sort_visitor: public dfs_visitor<> {
	...
	topo_sort_visitor(OutputIterator _iter, bool &_isDAG) :
			m_iter(_iter), isDAG(_isDAG) {
		isDAG = true;
	}

	template<typename Edge, typename Graph>
	void back_edge(const Edge&, Graph& g) {
		isDAG = false;
	}
...
bool topological_sort(VertexListGraph& g, OutputIterator result,
		const bgl_named_params<P, T, R>& params) {
	typedef topo_sort_visitor<OutputIterator> TopoVisitor;
	bool isDAG;
	TopoVisitor tv(result, isDAG);
	depth_first_search(g, params.visitor(tv));
	return isDAG;
}

template<typename VertexListGraph, typename OutputIterator>
bool topological_sort(VertexListGraph& g, OutputIterator result) {
	return topological_sort(g, result, bgl_named_params<int, buffer_param_t>(0)); // bogus
}
